...
1. Repo-local governance (private, primary):
   https://github.com/NeuroLift-Technologies/auth0-assistant0/blob/main/NLT-DEV-OTOI.md
   Public mirror (if the link above returns 404):
   https://github.com/NeuroLift-Technologies/auth0-assistant0/blob/main/governance/NLT-DEV-OTOI.md
2. Internal gateway (private, primary):
   https://github.com/NeuroLift-Technologies/auth0-assistant0/blob/main/AGENTS.md
   Public mirror (if the link above returns 404):
   https://github.com/NeuroLift-Technologies/auth0-assistant0/blob/main/governance/AGENTS.md
...
Format reference: `NeuroLift-Technologies/auth0-assistant0` templates directory.
```
...
Format reference: `templates/escalation.md` in `NeuroLift-Technologies/auth0-assistant0`.
```

### Step 5: Grant GitHub App Access to `auth0-assistant0`

Coding agents (e.g., GitHub Copilot, Codex) must be able to read the governance files in
`auth0-assistant0` from within the new repository. If the GitHub App installation is scoped
to **"Selected repositories"**, you must add `auth0-assistant0` to its access list.
...
3. Under **Repository access → Selected repositories**, add **`auth0-assistant0`**.
4. Click **Save**.
...
> in `auth0-assistant0` for full instructions.
...
- [ ] GitHub App has access to `auth0-assistant0` (or public mirror fallback is in place)

---

## Validation

Run the org-level validation script to check `auth0-assistant0` is healthy:

```bash
git clone https://github.com/NeuroLift-Technologies/auth0-assistant0
cd auth0-assistant0
