...
|---|---|---|---|
| **Public governance identity** | `NeuroLift-Technologies/auth0-assistant0` | All agents, public | Solidarity Framework principles, HAIEF attribution, org profile |
| **Private operational governance** | `NeuroLift-Technologies/auth0-assistant0` | Internal coding agents only | TOI-OTOI contracts, internal procedures, escalation templates, agent registration |
...
## `auth0-assistant0` File Structure (from nlt-business-agents)
...
| Source (nlt-business-agents) | Destination (auth0-assistant0) | Change |
|---|---|---|
...
| File | Content |
|---|---|
| `AGENTS.md` | Thin gateway — points to `auth0-assistant0` for internal governance, includes public Solidarity Framework principles |
...
1. **Create `NeuroLift-Technologies/auth0-assistant0`** (private repo, org members only)
2. **Populate from nlt-business-agents** using the mapping table above
3. **Update `nltotoi.json`** in `auth0-assistant0` to scope to org:
   ```json
   "repository": {
     "name": "NeuroLift-Technologies/auth0-assistant0",
     "purpose": "Repository-specific coding agent governance for auth0-assistant0",
     "mode": "production"
   }
   ```
...
1. Repo-local governance: https://github.com/NeuroLift-Technologies/auth0-assistant0/blob/main/NLT-DEV-OTOI.md
